...
运行这个项目需要**同时开着多个终端窗口**，每个窗口干不同的事。很多人会搞混"命令该输到哪个窗口"，下面讲清楚。

### 怎么打开终端？

**Mac 用户**：
- 方法 1：按 `Command + 空格`，输入 `Terminal`（或"终端"），回车
- 方法 2：打开 `访达` → `应用程序` → `实用工具` → `终端`
- 方法 3：在 Cursor 编辑器里，按 `` Ctrl + ` ``（键盘左上角的反引号键）打开内置终端
...
┌─────────────────────────────────────────────────────────────┐
│  窗口 1：后端服务（一直运行，不要关）                           │
│  标志：屏幕上会一直显示 "Uvicorn running on ..."              │
│  用途：跑后端 Python 服务                                     │
├─────────────────────────────────────────────────────────────┤
│  窗口 2：前端服务（一直运行，不要关）                           │
│  标志：屏幕上会一直显示 "Local: http://localhost:5173/"       │
│  用途：跑前端 Vue 服务                                        │
├─────────────────────────────────────────────────────────────┤
│  窗口 3：自由窗口（输入各种命令用）                             │
│  标志：显示 $ 或 % 等待你输入，没有服务在运行                   │
│  用途：安装依赖、git 操作、杀进程等                            │
└─────────────────────────────────────────────────────────────┘
```
...
- 如果窗口里在跑服务（不停地刷日志），**不要在这里输命令**，你输的东西会打断服务
- 命令应该输到**空闲窗口**——就是显示 `$` 或 `%` 符号、等着你输入的那个

```
空闲窗口长这样（可以输命令）：
chichi@MacBook-Air 项目文件夹 %  ← 光标在这里闪，可以输入

忙碌窗口长这样（不要输命令）：
INFO:     Uvicorn running on http://0.0.0.0:8000
INFO:     127.0.0.1:52341 - "GET /api/..." 200 OK  ← 在刷日志
```

### 怎么新开一个终端窗口？

- **Mac 终端**：按 `Command + T`（新标签页）或 `Command + N`（新窗口）
- **Cursor 内置终端**：点终端面板右上角的 `+` 号

### 如果输错窗口了怎么办？

按 `Ctrl + C` 可以中断当前命令。如果不小心把服务关了，重新启动就行：
...
改了代码后想保存到 GitHub？按下面步骤操作。**所有 git 命令都在"自由窗口"（窗口 3）里输入。**

### 首次推送（全新项目，GitHub 上还没有仓库）

#### 1. 在 GitHub 上创建仓库

1. 打开 https://github.com ，登录你的账号
2. 点右上角 `+` → `New repository`
3. 填写仓库名（比如 `AI-customer-robot`）
4. **不要勾选** "Add a README file"（因为我们本地已经有了）
5. 点 `Create repository`

#### 2. 配置 SSH 密钥（只需做一次）

> SSH 密钥就像一把钥匙，让你的电脑和 GitHub 之间互相认识，以后推代码不用每次输密码。
...
如果显示 `No such file`，说明还没有，需要生成一个：
...
终端会输出一长串以 `ssh-ed25519` 开头的文字，**全部复制**。

打开 https://github.com/settings/keys → 点 `New SSH key` → 标题随便填 → 把复制的内容粘贴到 Key 框里 → 点 `Add SSH key`
...
看到 `Hi 你的用户名! You've successfully authenticated` 就说明成功了。

#### 3. 初始化并推送

在**自由窗口（窗口 3）**里，确保你在项目根目录下，然后依次运行：
...
# 创建第一次提交
git commit -m "first commit: 智能客服系统"
...
### 日常推送（改了代码后想更新到 GitHub）

每次修改代码后，只需要在**自由窗口（窗口 3）**里运行 3 条命令：

```bash
# 第 1 步：添加所有改动的文件
git add .

# 第 2 步：提交，引号里写你改了什么（随便写，给自己看的）
git commit -m "修复了xx bug" 
...
你不在项目根目录。先 `cd` 到项目文件夹：
```bash
cd /你的项目路径
```

**Q: 提示 "Permission denied (publickey)"**
SSH 密钥没配好。回到上面"配置 SSH 密钥"那一步重新来。

**Q: 提示 "everything up-to-date"**
说明没有新的改动需要推送，你的代码已经是最新的了。

**Q: 提示 "Please tell me who you are"**
Git 不知道你是谁，设置一下名字和邮箱：
```bash
